...
class Book:
    ...
    def __init__(self):
        ...
        self.image_flags = Book.DefaultImageFlags
    ...
    def save(self, filename):
        ...
        root.setAttribute('imageFlags', self.image_flags)

        for filenameImg in self.images:
            item_img = document.createElement('image')
            root.appendChild(item_img)
            item_img.setAttribute('filename', filenameImg)

        text_xml = document.toString(4).toUtf8()

        try:
            file_xml = open(unicode(filename), 'w')
            file_xml.write(text_xml)
            file_xml.close()
        except IOError:
    ...
    def load(self, filename):
        try:
            file_xml = open(unicode(filename), 'r')
        ...
        if not document.setContent(QtCore.QString.fromUtf8(text_xml)):
        ...
        self.image_flags = int(root.attribute('imageFlags', str(Book.DefaultImageFlags)))
        self.filename = filename
        self.modified = False
        self.images = []
    ...
    def add_image_dirs(self, directories):
        filenames = []
    ...
    def add_image_files(self, filenames):
        # print len(filenames)
    ...
    def is_image_file(filename):
        ...
        return os.path.isfile(filename) and file_type in image_exts
