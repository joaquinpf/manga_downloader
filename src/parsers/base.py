...
#		img_url = 'http://' + urllib.quote(img_url.split('//')[1])
    ...
    def processChapter(self, downloadThread, current_chapter):
        ...
        if self.useShortName_FLAG:
            ...
            manga_chapter_prefix = fixFormatting(self.manga, self.spaceToken) + '.' +  self.site + '.' + zeroFillStr(fixFormatting(self.chapters[current_chapter][1].encode('utf-8'), self.spaceToken), 3)


        # we already have it
        if os.path.exists(os.path.join(self.downloadPath, manga_chapter_prefix) + self.downloadFormat) and self.overwrite_FLAG == False:
            print(self.chapters[current_chapter][1].encode('utf-8') + ' already downloaded, skipping to next chapter...')
            return
    ...
    class DownloadChapterThread( threading.Thread ):
        ...
        def run (self):
            try:
                ...
                if (self.siteParser.verbose_FLAG):
                    traceback.print_exc()
                chapterThreadSemaphore.release()
